(function () {
  ...
  function detectLocation() {
    ...
    if (locationContainer) {
      locationContainer.innerHTML = '<div class="location-loading"><h3>Detecting your location...</h3><p>Please allow location access to see personalized content for your area.</p></div>';
    }
    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(
        function(position) {
          ...
          getLocationFromCoordinates(lat, lng, function(actualLocation) {
            generateLocationContent(actualLocation, lat, lng);
          });
        }, 
        function(error) {
  ...
  // Enhanced location detection from coordinates using reverse geocoding
  function getLocationFromCoordinates(lat, lng, callback) {
    ...
    fetch(url)
      ...
      .then(data => {
        ...
        // Try to find a matching location in our data
        var detectedLocation = findBestLocationMatch(location, state, lat, lng);
        callback(detectedLocation);
      })
      .catch(error => {
        console.log('Reverse geocoding failed:', error);
        // Fallback to coordinate-based detection
        var fallbackLocation = getLocationFromCoordinatesFallback(lat, lng);
        callback(fallbackLocation);
      });
  }

  // Find the best matching location in our data
  function findBestLocationMatch(location, state, lat, lng) {
    ...
    for (var i = 0; i < cityVariations.length; i++) {
      if (locationData[cityVariations[i]]) {
        return cityVariations[i];
      }
    }
  ...
  // Fallback coordinate-based location detection
  function getLocationFromCoordinatesFallback(lat, lng) {
    ...
    var closestCity = 'Chicago'; // default
    var minDistance = Infinity;
    
    for (var city in cityCoordinates) {
      var cityLat = cityCoordinates[city].lat;
      var cityLng = cityCoordinates[city].lng;
  ...
  function generateLocationContent(location, lat, lng) {
    ...
    var container = document.getElementById('location-content');
    if (!container) return;

    container.innerHTML = '';

    // Add location header with GPS detection indicator
    var locationDisplay = location;
    if (lat && lng) {
      locationDisplay += ' (Detected via GPS)';
    }
    
    var locationHeader = document.createElement('div');
    locationHeader.className = 'location-header';
    locationHeader.innerHTML = '<h3>📍 ' + locationDisplay + ', ' + content.state + '</h3>';
  ...
  window.retryLocationDetection = retryLocationDetection;
